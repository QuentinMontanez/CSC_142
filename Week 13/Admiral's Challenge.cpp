...
int main() {
    ...
    double efficiency = (fuel / spaceship_weight) * 600;

    
    double speed = (fuel * 50000) / (crew_size * spaceship_weight);

    
    double travel_time = (distance * 2500000) / speed;
    int travel_days = static_cast<int>(std::ceil(travel_time));

    
    bool is_viable = (efficiency >= 50.0) &&
                     (life_support >= travel_days + 10) &&
                     (crew_size >= 2 && crew_size <= 8) &&
                     (spaceship_weight < 750);
